...
@property (nonatomic, strong) NSLayoutConstraint *rightButtonEmojiWC;
@property (nonatomic, strong) NSLayoutConstraint *rightButtonEmojiMarginWC;
@property (nonatomic, strong) NSLayoutConstraint *rightButtonEmojiTopMarginC;
@property (nonatomic, strong) NSLayoutConstraint *rightButtonEmojiBottomMarginC;
...
- (void)slk_commonInit
{
    ...
    [self addSubview:self.rightButtonEmoji];
...
{
    if (!_rightButtonEmoji) {
        _rightButtonEmoji = [UIButton buttonWithType:UIButtonTypeSystem];
        _rightButtonEmoji.translatesAutoresizingMaskIntoConstraints = NO;
        _rightButtonEmoji.titleLabel.font = [UIFont systemFontOfSize:15.0];
    }
    return _rightButtonEmoji;
}
...
- (UIView *)editorContentView
{
    if (!_editorContentView) {
        ...
        [_editorContentView addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"H:|-(left)-[leftButton(60)]-(left)-[label(>=0)]-(right)-[rightButton(0)]-(<=right)-|" options:0 metrics:metrics views:views]];
...
- (CGFloat)slk_appropriateRightButtonEmojiWidth
{
    
    return [self.rightButtonEmoji intrinsicContentSize].width;
...
- (void)slk_setupViewConstraints
{
    NSDictionary *views = @{@"textView": self.textView,
                            ...
                            @"rightButtonEmoji": self.rightButtonEmoji,
    ...
    [self addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"H:|-(left)-[leftButton(0)]-(<=left)-[textView]-(right)-[rightButtonEmoji(35)]-(right)-[rightButton(0)]-(right)-|" options:0 metrics:metrics views:views]];
    ...
    [self addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"V:|-(>=0)-[rightButtonEmoji]-(<=0)-|" options:0 metrics:metrics views:views]];
    ...
    NSLayoutConstraint *rightbutton_emoji_centreHorizontallyConstraint = [NSLayoutConstraint
                                                        constraintWithItem:self.rightButtonEmoji
                                                        attribute:NSLayoutAttributeCenterY
                                                        relatedBy:NSLayoutRelationEqual
                                                        toItem:self
                                                        attribute:NSLayoutAttributeCenterY
                                                        multiplier:1.0
                                                        constant:(self.frame.size.height-35)/2];
    
//    [self addConstraint:leftbutton_centreHorizontallyConstraint];
    [self addConstraint:rightbutton_emoji_centreHorizontallyConstraint];
    ...
    //self.rightButtonEmojiWC = [self slk_constraintForAttribute:NSLayoutAttributeWidth firstItem:self.rightButtonEmoji secondItem:nil];
    //self.rightButtonEmojiMarginWC = [self slk_constraintsForAttribute:NSLayoutAttributeTrailing][0];
    
    //self.rightButtonEmojiTopMarginC = [self slk_constraintForAttribute:NSLayoutAttributeTop firstItem:self.rightButtonEmoji secondItem:self];
    //self.rightButtonEmojiBottomMarginC = [self slk_constraintForAttribute:NSLayoutAttributeBottom firstItem:self.rightButtonEmoji secondItem:self];

    
   // [self.rightButtonEmoji setBackgroundColor:[UIColor redColor]];
...
- (void)slk_updateConstraintConstants
{
    ...
    if (self.isEditing)
    {
        ...
        self.rightButtonEmojiWC.constant = 35;
       // self.rightButtonEmojiMarginWC.constant = [self slk_appropriateRightButtonMargin];
       ...
       // self.rightButtonEmojiTopMarginC.constant = rightVerMargin;
        //self.rightButtonEmojiBottomMarginC.constant = rightVerMargin;
        //self.rightButtonEmojiTopMarginC.constant = (self.frame.size.height - 35)/2;
        //self.rightButtonEmojiBottomMarginC.constant = (self.frame.size.height - 35)/2;
    }
}
