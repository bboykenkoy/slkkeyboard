...
#import "EmojiCustomCollectionViewCell.h"
...
@property  UIView * paintView;
@property  UICollectionView * collection;
@property  UICollectionView * collectionChidl;
@property  UIView * line;
@property NSIndexPath* oldCell;
...
- (void)viewDidLoad
{
    ...
    _currentKeyboardHeight = 250;
    _keyboardHeight= 250;
    
    _paintView=[[UIView alloc]initWithFrame:CGRectMake(0, self.view.frame.size.height - _currentKeyboardHeight,  self.view.frame.size.width ,_currentKeyboardHeight)];
    ...
    _collection=[[UICollectionView alloc] initWithFrame:CGRectMake(0, 0,  self.view.frame.size.width ,_paintView.frame.size.height - 44) collectionViewLayout:layout];
    [_collection setDataSource:self];
    [_collection setDelegate:self];
    [_collection setPagingEnabled:true];
    _collection.backgroundColor = [UIColor whiteColor];
    [_collection registerNib:[UINib nibWithNibName:@"EmojiCustomCollectionViewCell" bundle:nil] forCellWithReuseIdentifier:@"emojicell"];
    
    [self.paintView addSubview:_collection];
    
    [_paintView setHidden:true];
    
    _line = [[UIView alloc]initWithFrame:CGRectMake(0,_collection.frame.size.height,  self.view.frame.size.width , 0.5)];
    _line.backgroundColor = [UIColor grayColor];
    _line.alpha = 0.3;
    [_paintView addSubview:_line];
    
    UICollectionViewFlowLayout *layout1 =[[UICollectionViewFlowLayout alloc] init];
    layout1.scrollDirection = UICollectionViewScrollDirectionHorizontal;
    
    _collectionChidl =[[UICollectionView alloc] initWithFrame:CGRectMake(0,_collection.frame.size.height + 1,  self.view.frame.size.width , 43.5) collectionViewLayout:layout1];
    [_collectionChidl setDataSource:self];
    [_collectionChidl setDelegate:self];
    
    [_collectionChidl registerNib:[UINib nibWithNibName:@"EmojiCollectionViewCell" bundle:nil] forCellWithReuseIdentifier:@"cellid"];
    _collectionChidl.backgroundColor  = [UIColor whiteColor];
    [self.paintView addSubview:_collectionChidl];
    
    
    _oldCell = [NSIndexPath indexPathWithIndex:0];
...
- (void)updateFrameViewEmoji{
    
    _paintView.frame = CGRectMake(0, self.view.frame.size.height - _currentKeyboardHeight,  self.view.frame.size.width ,_currentKeyboardHeight);
    _collection.frame = CGRectMake(0, 0,  self.view.frame.size.width ,_paintView.frame.size.height - 44);
    _line.frame = CGRectMake(0,_collection.frame.size.height,  self.view.frame.size.width , 0.5);
    _collectionChidl.frame = CGRectMake(0,_collection.frame.size.height + 0.5,  self.view.frame.size.width , 43.5);
...
- (SLKTextInputbar *)textInputbar
{
    if (!_textInputbar) {
        ...
        [_textInputbar.rightButtonEmoji addTarget:self action:@selector(didPressRightButtonEmoji:) forControlEvents:UIControlEventTouchUpInside];
...
- (UIButton *)rightButtonEmoji
{
    return _textInputbar.rightButtonEmoji;
}
...
- (void)didPressRightButtonEmoji:(id)sender{
...
//        _singleTapGesture = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(slk_didTapScrollView:)];
//        _singleTapGesture.delegate = self;
//        [_singleTapGesture requireGestureRecognizerToFail:_scrollViewProxy.panGestureRecognizer];
//        [_scrollViewProxy addGestureRecognizer:self.singleTapGesture];
    ...
    if (!self.isPresentedInPopover && ![self ignoreTextInputbarAdjustment]) {
        ...
        if (self.keyboardHC.constant == 0){
            _keyboardHC.constant = 250;
            [_paintView setHidden:false];
        }
        //[self.view setNeedsLayout];
        
        self.scrollViewHC.constant = [self slk_appropriateScrollViewHeight];
        [self updateFrameViewEmoji];
        
    }
    
    if (!self.isShowEmoji) {
        [self.rightButtonEmoji setImage:[UIImage imageNamed:@"ic_emoji"] forState:UIControlStateNormal];
        [self.rightButtonEmoji setTintColor:[UIColor darkGrayColor]];
    }else{
        [self.rightButtonEmoji setImage:[UIImage imageNamed:@"ic_keyboard"] forState:UIControlStateNormal];
        [self.rightButtonEmoji setTintColor:[UIColor darkGrayColor]];
    }
...
//    [self didPasteMediaContent:userInfo];
    // No implementation here. Meant to be overriden in subclass.
}
...
- (void)slk_didTapScrollView:(UIGestureRecognizer *)gesture
{
    ...
    //    [[NSNotificationCenter defaultCenter] postNotificationName:@"DidHideEmoji" object:self.textView userInfo:nil];
    
}



#pragma mark phuonghv add function
- (void)showEmoji{
...
- (void)slk_willShowOrHideKeyboard:(NSNotification *)notification
{
    ...
    if (_isShowEmoji) {
        self.keyboardHC.constant = _currentKeyboardHeight;
        
    }else{
        self.keyboardHC.constant = [self slk_appropriateKeyboardHeightFromNotification:notification];
        if ([self slk_appropriateKeyboardHeightFromNotification:notification] > 0) {
            _currentKeyboardHeight = [self slk_appropriateKeyboardHeightFromNotification:notification];
        }
    }
...
- (void)textViewDidBeginEditing:(SLKTextView *)textView
{
    ...
    _keyboardHC.constant = _currentKeyboardHeight;
    self.scrollViewHC.constant = [self slk_appropriateScrollViewHeight];
    //[self.view setNeedsLayout];
    [self.paintView setHidden:true];
    //[self updateFrameViewEmoji];
...
- (NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section;
{
    if (collectionView == _collection){
        return _arrEmoji.count;
    }
    else if(collectionView ==  _collectionChidl){
        return _arrEmoji.count;
    }
    return 0;
}
...
{
    if (collectionView == _collection) {
        EmojiCustomCollectionViewCell *cell= [collectionView dequeueReusableCellWithReuseIdentifier:@"emojicell" forIndexPath:indexPath];
        cell.tag = indexPath.row;
      //  NSDictionary * dic = [_arrEmoji objectAtIndex:indexPath.row];
        cell.emojis = [_arrEmoji objectAtIndex:indexPath.row];
       // cell.catenory = [dic objectForKey:@"emojiImg"];
        cell.delegate = self;
        return cell;
    }
    else if (collectionView == _collectionChidl) {
        ...
        NSMutableArray * dic = [_arrEmoji objectAtIndex:indexPath.row];
        NSString *nameemojis = [[dic objectAtIndex:0] objectForKey:@"emojiImg"];
        //cell.img.image  = [UIImage imageNamed:[NSString stringWithFormat:@"%@",nameemojis]];
        
        if ([AppManger.cache objectForKey:nameemojis]) {
            dispatch_async(dispatch_get_main_queue(), ^{
                cell.img.image = (UIImage*)[AppManger.cache objectForKey:nameemojis];
            });
        }else{
            dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                
                __block UIImage *image = [UIImage imageNamed:nameemojis];
                if (image) {
                    image = [Utils imageWithImage:image scaledToWidth:100];
                }
                dispatch_async(dispatch_get_main_queue(), ^{
                    cell.img.image = image;
                    [AppManger.cache setObject:image forKey:nameemojis];
                    image = nil;
                });
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    if(scrollView == _collection){
        CGRect visibleRect = (CGRect){.origin = self.collection.contentOffset, .size = self.collectionView.bounds.size};
        CGPoint visiblePoint = CGPointMake(CGRectGetMidX(visibleRect), CGRectGetMidY(visibleRect));
        NSIndexPath *visibleIndexPath = [self.collection indexPathForItemAtPoint:visiblePoint];
        
        [self.collectionChidl scrollToItemAtIndexPath:visibleIndexPath atScrollPosition:UICollectionViewScrollPositionNone animated:YES];
        
        EmojiCollectionViewCell * cell = (EmojiCollectionViewCell*)[_collectionChidl cellForItemAtIndexPath:_oldCell];
        cell.backgroundColor = [UIColor whiteColor];
        cell = (EmojiCollectionViewCell*)[_collectionChidl cellForItemAtIndexPath:visibleIndexPath];
        
        cell.backgroundColor = [UIColor darkGrayColor];
        _oldCell = visibleIndexPath;
    }
...
{
    if (collectionView == _collectionChidl) {
...
-(void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    if (collectionView == _collectionChidl) {
        EmojiCollectionViewCell * cell2 = (EmojiCollectionViewCell*)[_collectionChidl cellForItemAtIndexPath:_oldCell];
        cell2.backgroundColor = [UIColor whiteColor];
        [_collection scrollToItemAtIndexPath:indexPath atScrollPosition:UICollectionViewScrollPositionNone animated:YES];
        EmojiCollectionViewCell * cell = (EmojiCollectionViewCell*)[_collectionChidl cellForItemAtIndexPath:indexPath];
...
- (void) getIndexCell:(NSNotification *) notification {
    
    NSDictionary *userInfo = notification.userInfo;
    int index1 = [(NSNumber*)[userInfo objectForKey:@"index1"] intValue];
    int index2 =  [(NSNumber*)[userInfo objectForKey:@"index2"] intValue];
...
- (void)setDataEmoji: (NSMutableArray*) emojiArray;{
    _arrEmoji = emojiArray;
    [self.collection reloadData];
    [self.collectionChidl reloadData];
}

-(void)didSelectCellEmojiIndexpath:(NSString *)data category:(NSString*)category{
    NSLog(@"%@",data);
    
    NSMutableDictionary *userInfo = [NSMutableDictionary dictionary];
    [userInfo setObject:data forKey:@"emojiData"] ;
    [userInfo setObject:category forKey:@"category"] ;
    
//    [[NSNotificationCenter defaultCenter] postNotificationName:
//     @"didSelectEmoji" object:nil userInfo:userInfo];
}
